...
#===============================================================================================================
extra_simulation_time = 0.050 #extra time brian simulation will run after last signal time in seconds
...
input_hidden_time_constants_FR_up_exc = numpy.random.uniform(low = min_time_constant, high=max_time_constant, size = int(Hidden_neurons/2))
...
input_hidden_time_constants_FR_dn_exc = numpy.random.uniform(low = min_time_constant, high=max_time_constant, size = int(Hidden_neurons/2))
...
input_hidden_time_constants_FR_up = np.concatenate((input_hidden_time_constants_FR_up_exc,input_hidden_time_constants_FR_up_inh))
input_hidden_time_constants_FR_dn = np.concatenate((input_hidden_time_constants_FR_dn_inh,input_hidden_time_constants_FR_dn_exc))

#====================================
# Get all time constants in a vector
#====================================
input_hidden_time_constants = np.concatenate((input_hidden_time_constants_FR_up,input_hidden_time_constants_FR_dn))


#====================================
# Weights for Fast Ripple channels
#====================================
input_hidden_weights_FR_up_exc = rdm.choices(possible_weights_exc, k = int(Hidden_neurons/2))
input_hidden_weights_FR_dn_inh = [-1*x for x in input_hidden_weights_FR_up_exc]
input_hidden_weights_FR_dn_exc = rdm.choices(possible_weights_exc, k = int(Hidden_neurons/2))
...
for cp, current_patient in enumerate(list_patients):
    ...
    Test_Results['Info']['Recordings'] = np.array(['%s-recorded data' %recording_type])
    ...
    for current_channel in range(number_of_channels):
        ...
        Ecog_signal_raw =  Data['%s_Ecog_signal' %recording_prefix][current_channel,:]
        ...
        Test_Results['Info']['Recording_duration'] = np.array([time_vector[-1]])
        ...
        Input_Hidden_layer.I_tau = getTauCurrent(input_hidden_time_constants*1e-3, True) * amp
        ...
        #duration = 10
        print('Running SNN for Patient %s channel %s '%(current_patient, current_channel))
        ...
        for m in range(start_teacher.size):
            ...
            neurons_spiking_at_HFO.append(((Spike_Monitor_Hidden.i/1)[range_of_spikes_HFO]).size)
            neurons_spiking_at_noise.append(((Spike_Monitor_Hidden.i/1)[range_of_spikes_noise]).size)    
        ...
        HFO_detection = detect_HFO(trial_duration = duration, 
                                   ...
                                   neuron_spike_monitor = (Spike_Monitor_Hidden.i/1),
                                   step_size = step_size,
        ...
        Test_Results['HFO_periods']['channel_%s' %current_channel] = HFO_detection['periods_HFO']
